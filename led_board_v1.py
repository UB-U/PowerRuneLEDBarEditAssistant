...
def arr_Output(on_indices):
    ...
    return (f"// 共{len(segs)}段  \n"
            f"const uint8_t seg_count = {len(segs)};\n"
            f"const uint16_t led_seg[{len(segs)}][2] = {{{body}}};")
...
class LEDBoardGUI:
    ...
    def _build(self):
        ...
        for text, cmd in [
            ("全选", self._select_all), ("清空", self._clear_all),
...
# ============================================================
    ...
    def _select_all(self):
        for idx in range(LEDNUMSUM):
            self.on_leds.add(idx)
            _ = self.canvas.itemconfig(self.rect_ids[idx],  # pyright: ignore[reportArgumentType]
    ...
    def _show_output(self):
        if hasattr(self, "_output_window") and self._output_window.winfo_exists():
        ...
        ok = sorted(self.on_leds) == decode_continuous(segs)
